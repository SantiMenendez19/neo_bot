...
async def on_message(message):
    ...
    respuesta = ["Si", "No", "Tal vez", "Nunca", "Seguramente", "Nah", "100% Seguro"]

    # Frases aleatorias
    frases = ["No mandes deploy los viernes", "Hola", "Adios"]
    ...
    if message.content == "!roll":
        ...
        msg = ("El usuario {0.author.mention} saco el numero: " + num).format(message)
        await message.channel.send(msg)
    ...
    if message.content.startswith("!pregunta"):
        preg = re.compile(r"(!pregunta)").sub(
            "",
            message.content,
        )
    ...
    if message.content.startswith("!imagen"):
        image_list = []
        palabra_imagen = re.compile(r"(!imagen)").sub(
            "",
            message.content,
        )
        url_google = "https://www.google.com/search?q=" + palabra_imagen + "&source=lnms&tbm=isch"
        ...
        e = discord.Embed()
        e.set_image(url=image_list[random.randrange(1 + (len(image_list) - 1))])
        ...
        if jug[turno_jug] == message.author.id:
            ...
            if tablero[(int(pos) // 3)][(int(pos) % 3)] == "X" or tablero[(int(pos) // 3)][(int(pos) % 3)] == "O":
                ...
                if (
                    ...
                    and tablero[0][0] == tablero[0][2]
                    or tablero[1][0] == tablero[1][1]
                    and tablero[1][0] == tablero[1][2]
                    or tablero[2][0] == tablero[2][1]
                    and tablero[2][0] == tablero[2][2]
                    or tablero[0][0] == tablero[1][0]
                    and tablero[0][0] == tablero[2][0]
                    or tablero[0][1] == tablero[1][1]
                    and tablero[0][1] == tablero[1][2]
                    or tablero[0][2] == tablero[1][2]
                    and tablero[0][2] == tablero[2][2]
                    or tablero[0][0] == tablero[1][1]
                    and tablero[0][0] == tablero[2][2]
                    or tablero[2][0] == tablero[1][1]
                    and tablero[2][0] == tablero[0][2]
                ):
